...
public:
  void
  run(std::string file_name)
  {
    std::string name_space = "/example/testApp/test_files/";
    name_space += file_name;
    Name interestName(name_space);
...
private:
  ...
  onData(const Interest& interest, const Data& data) const
  {
    ...
    std::string file_path("/home/ndn1/test_files2/");
    file_path += id; 
    file_path += ".txt";
    
    Block content = data.getContent(); //1
    std::string payload = std::string(content.value_begin(), content.value_end());//3
